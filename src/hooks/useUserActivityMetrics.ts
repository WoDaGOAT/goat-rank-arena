  ...
  const { data: userActivityMetrics } = useQuery({
    ...
    queryFn: async (): Promise<UserActivityMetrics> => {
      try {
        ...
        if (dauError) {
          console.warn('DAU query failed:', dauError);
        }
        ...
        if (wauError) {
          console.warn('WAU query failed:', wauError);
        }
        ...
        if (mauError) {
          console.warn('MAU query failed:', mauError);
        }
        ...
        if (oldActiveError) {
          console.warn('Old active users query failed:', oldActiveError);
        }
        ...
        console.warn('User activity metrics error:', error);
        return {
          dau: 0,
          wau: 0,
          mau: 0,
