...
import BulkEnrichmentApproval from "./BulkEnrichmentApproval";
...
const AthleteEnrichment = ({ onEnrichmentComplete }: AthleteEnrichmentProps) => {
  ...
  if (showBulkApproval && results?.athlete_suggestions) {
    return (
      <div className="space-y-6">
        ...
        <BulkEnrichmentApproval
          suggestions={results.athlete_suggestions}
  ...
  return (
    <Card className="w-full">
      <CardHeader>
        ...
        <CardDescription>
          Scan for high-quality enrichment opportunities and approve them in bulk.
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-2xl font-bold text-purple-600">
                        {results.athlete_suggestions.reduce((sum, athlete) => sum + athlete.suggestions.length, 0)}
                      </p>
                    </div>
                  </div>
                </CardContent>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  >
                    ...
                    Review & Apply All ({results.athlete_suggestions.reduce((sum, athlete) => sum + athlete.suggestions.length, 0)} suggestions)
                  </Button>
                </div>
                  ...
                  {results.athlete_suggestions.map((athlete, index) => (
                    <div key={index} className="flex items-center justify-between p-3 bg-green-50 rounded border border-green-200">
                      ...
                      <div className="text-sm text-muted-foreground">
                        {athlete.suggestions.map(s => s.field).join(', ')}
                      </div>
                    </div>
                  ))}
                </div>
